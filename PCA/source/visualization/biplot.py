...
class BiplotVisualizer(BasePlotter):
    ...
    def create_biplot(self, pca_model, x_standardized, data, feature_to_group=None,
                      ...
                      enable_feature_grouping=False, significance_threshold=0.2, focus_on_loadings=False, target=None):
        """
        ...
        if target:
            ...
            target_colors = {t: plt.cm.tab10(i / len(unique_targets)) for i, t in enumerate(unique_targets)}

            # Plot data points with colors based on target variable
            for t in unique_targets:
                indices = data[target] == t
                self.ax.scatter(
                    pca_transformed[indices, 0],
                    pca_transformed[indices, 1],
                    label=f"{target}: {t}",
                    color=target_colors[t],
                    alpha=0.7
                )
        else:
            # Default scatter plot if no target variable is specified
            self.ax.scatter(pca_transformed[:, 0], pca_transformed[:, 1], alpha=0.7)
